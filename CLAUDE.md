...
*Phased arming:* the gate fails the merge only on **armed** steps (currently `cargo fmt`, `clippy` incl. `pedantic`, `ruff lint`, `ruff format`, `mypy --strict`, `DRY`, `Rust coverage ≥ 80%` — verified clean repo-wide). The rest run **warn-only** (`continue-on-error`) while pre-existing debt is cleared — file size, complexity, Python coverage, mutation — and are armed one at a time as each is cleaned. See the header comment in the workflow.
