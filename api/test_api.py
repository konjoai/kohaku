...
def _reset_state():
    ...
    rest.enriched = EnrichedMemoryStore(capacity=rest.episodic._capacity, dims=rest.dims)
...
def test_memories_store_returns_id_and_metadata(client: TestClient):
    r = client.post("/memories/store", json={
        "label": "user fact A", "input": "the cat sat on the mat",
...
def test_memories_store_rejects_bad_importance(client: TestClient):
    r = client.post("/memories/store", json={
        "label": "x", "input": "hi", "importance": 1.5,
    })
    assert r.status_code == 422
...
def test_memories_query_filters_expired(client: TestClient):
    ...
    client.post("/memories/store", json={
        "label": "stale", "input": "stale memory phrase",
        "valid_from": "2020-01-01T00:00:00Z",
        "valid_until": "2020-12-31T00:00:00Z",
    })
    # And a live one
    client.post("/memories/store", json={
        "label": "live", "input": "live memory phrase",
    })
    r = client.post("/memories/query", json={
        "input": "memory phrase", "top_k": 5, "reinforce_hits": False,
    })
    assert r.status_code == 200
...
def test_memories_query_include_expired_returns_them(client: TestClient):
    client.post("/memories/store", json={
        "label": "stale", "input": "stale",
        "valid_from": "2020-01-01T00:00:00Z",
        "valid_until": "2020-12-31T00:00:00Z",
    })
    r = client.post("/memories/query", json={
        "input": "stale", "top_k": 5, "include_expired": True, "reinforce_hits": False,
    })
    labels = [row["label"] for row in r.json()["results"]]
...
def test_memories_query_source_filter(client: TestClient):
    client.post("/memories/store", json={
        "label": "u", "input": "user knowledge",
        "source": "user_input", "importance": 0.5,
    })
    client.post("/memories/store", json={
        "label": "a", "input": "user knowledge",
        "source": "agent_inference", "importance": 0.5,
    })
    r = client.post("/memories/query", json={
        "input": "user knowledge", "top_k": 5,
        "source_filter": "user_input", "reinforce_hits": False,
    })
...
def test_memories_query_sort_by_salience(client: TestClient):
    ...
    client.post("/memories/store", json={
        "label": "imp", "input": "fact about kohaku",
        "source": "user_input", "importance": 1.0,
    })
    client.post("/memories/store", json={
        "label": "weak", "input": "fact about kohaku",
        "source": "agent_inference", "importance": 0.1,
    })
    by_sal = client.post("/memories/query", json={
        "input": "fact about kohaku", "top_k": 2,
        "sort": "salience", "reinforce_hits": False,
    }).json()["results"]
...
def test_memories_list_returns_metadata(client: TestClient):
    for i in range(3):
        client.post("/memories/store", json={
            "label": f"m{i}", "input": f"phrase {i}",
            "source": "tool_result", "importance": 0.3 + i * 0.2,
        })
...
def test_memories_list_source_filter(client: TestClient):
    client.post("/memories/store", json={
        "label": "u", "input": "x", "source": "user_input"})
    client.post("/memories/store", json={
        "label": "w", "input": "y", "source": "web_search"})
...
def test_consolidate_endpoint_returns_report(client: TestClient):
    ...
    for i in range(4):
        client.post("/memories/store", json={
            "label": f"d{i}", "input": "the cat sat on the mat",
        })
...
def test_consolidate_no_merge_below_threshold(client: TestClient):
    ...
    for i in range(3):
        client.post("/memories/store", json={
            "label": f"x{i}", "input": f"unrelated phrase number {i}",
        })
